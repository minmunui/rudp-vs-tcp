...
class ServerLogAnalyzer:
    ...
    def _parse_block(self, block: str, protocol: str) -> Dict:
        ...
        # 파일 크기
        size_match = re.search(r'파일 크기:\s*([\d,]+)\s*bytes\s*\(([\d.]+)\s*MB\)', block)
        if size_match:
        ...
        if protocol == 'RUDP':
            ...
            if expected_match and received_match and lost_match:
                ...
                record['packet_loss_rate'] = (lost / expected * 100) if expected > 0 else 0
        ...
        # UDP 손실 정보
            ...
            if expected_match:
                ...
                record['packet_loss_rate'] = ((expected - received) / expected * 100) if expected > 0 else 0
    ...
    def analyze_by_protocol(self, records: List[Dict]) -> Dict:
        ...
        for protocol, data in protocol_stats.items():
            ...
            loss_rates = [r.get('packet_loss_rate', 0) for r in data if 'packet_loss_rate' in r]
    ...
    def export_csv(self, records: List[Dict], output_file: str):
        ...
        fields = ['protocol', 'filename', 'file_size_mb', 'transfer_speed', 'transfer_time',
                 'packet_loss_rate', 'packets_expected', 'packets_received', 'packets_lost']
    ...
    def print_summary(self, results: Dict):
        ...
        print(f"{'프로토콜':<12} {'전송 횟수':<12} {'평균 속도':<15} {'최소/최대':<20} {'평균 손실률':<15}")
        print(f"{'-'*80}")
        
        for protocol in sorted(results.keys()):
            stats = results[protocol]
            loss_str = f"{stats['avg_loss_rate']:.2f}%" if stats['avg_loss_rate'] > 0 else "-"
            
            print(f"{protocol:<12} {stats['count']:<12} "
