...
class BufferSizeComparison:
    ...
    def extract_stats(self, log_file: str):
        ...
        print(f"처리 완료: {log_file} → {protocol}, 버퍼 크기={buffer_size}, {len(speeds)}개 기록")
    ...
    def print_comparison(self, results: Dict):
        ...
        for protocol in protocols:
            ...
            # 가장 빠른 버퍼 크기
            best_buffer = max(results[protocol].items(), 
                            key=lambda x: x[1]['avg_speed'])
            
            print(f"• {protocol}: 버퍼 크기 {best_buffer[0]} "
    ...
    def export_csv(self, results: Dict, output_file: str):
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            ...
            # 헤더
            header = ['Protocol', 'BufferSize', 'Count', 'AvgSpeed', 'MinSpeed', 
                     'MaxSpeed', 'StdDev', 'AvgLoss', 'MinLoss', 'MaxLoss']
    ...
    def plot_graph(self, results: Dict, output_file: str = 'buffer_comparison.png'):
        ...
        if loss_protocols:
            ...
            ax2.set_title('Packet Loss Rate vs Buffer Size', fontsize=14, fontweight='bold')
            ax2.legend()
