...
class FileTransferClientProtocol(QuicConnectionProtocol):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.server_ack = asyncio.Event()  # 서버 응답 대기
    ...
    async def send_file(self, filename: str):
        ...
        metadata = {"filename": basename, "file_size": file_size}
        metadata_json = json.dumps(metadata).encode("utf-8")
        metadata_size = len(metadata_json).to_bytes(4, "big")

        # Send metadata size + metadata
        self._quic.send_stream_data(
            stream_id, metadata_size + metadata_json, end_stream=False
        )
        self.transmit()
        ...
        with open(filename, "rb") as f:
            ...
            while True:
                ...
                print(
                    f"\r전송 진행률: {progress:.1f}% ({sent}/{file_size} bytes)", end=""
                )
        ...
        try:
            ...
            logger.info(f"서버 확인 완료 (총 소요 시간: {self.transfer_time:.2f}초)")
        except asyncio.TimeoutError:
            self.transfer_time = time.time() - start_time
            logger.info(f"서버 응답 타임아웃 (소요 시간: {self.transfer_time:.2f}초)")
    ...
    def _process_received_file(self, stream_id: int):
        ...
        try:
            ...
            try:
                ...
                self._quic.send_stream_data(ack_stream_id, b"ACK", end_stream=True)
                self.transmit()
    ...
    async def _async_send_file(self, filename: str, host: str, port: int):
        ...
        try:
            ...
            async with connect(
                ...
                transfer_time = client.transfer_time
                success = True

                # Log transfer speed
                if transfer_time > 0 and file_size > 0:
                    speed = file_size / transfer_time / 1024 / 1024
            ...
            speed_mbps = (
                file_size / total_elapsed / 1024 / 1024 if total_elapsed > 0 else 0
            )

            return {
                "success": success,
                "transfer_time": total_elapsed,
                "speed_mbps": speed_mbps,
                "filesize": file_size,
                "losses": [],  # QUIC handles reliability internally
            }

        except Exception as e:
            logger.error(f"QUIC 전송 오류: {e}")
            return {
                "success": False,
                "transfer_time": 0,
                "speed_mbps": 0,
