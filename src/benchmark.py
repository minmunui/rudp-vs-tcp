#!/usr/bin/env python3
"""
TCP와 MIDTP의 전송 속도를 buffer_size별로 측정하여 CSV로 저장하는 스크립트
"""
...
OUTPUT_CSV = "benchmark_tcp_midtp_drop5_timeout1.csv"


def run_benchmark():
    """
    TCP와 MIDTP의 전송 속도를 측정하고 CSV 파일로 저장
    """
    results = []
    ...
    print(f"TCP 포트: {TCP_PORT}, MIDTP 포트: {MIDTP_PORT}")
    ...
    for multiplier in BUFFER_SIZE_MULTIPLIERS:
        ...
        for trial in range(1, TRIALS_PER_SIZE + 1):
            ...
            try:
                ...
                if midtp_result["success"]:
                    print(f"      MIDTP 완료: {midtp_result['speed_mbps']:.2f} MB/s")
                    results.append(
                        {
                            "protocol": "MIDTP",
                            "buffer_size": buffer_size,
                            "trial": trial,
                            "transfer_time": midtp_result["transfer_time"],
                            "speed_mbps": midtp_result["speed_mbps"],
                            "filesize": midtp_result["filesize"],
                            "success": True,
                            "loss_count": len(
                                [l for l in midtp_result["losses"] if len(l) > 0]
                            ),
                        }
                    )
                else:
    ...
    # 요약 통계 출력
    print("\n=== 요약 ===")
    for protocol in ["TCP", "MIDTP"]:
        protocol_results = [
            r for r in results if r["protocol"] == protocol and r["success"]
        ]
        if protocol_results:
            avg_speed = sum(r["speed_mbps"] for r in protocol_results) / len(
                protocol_results
            )
