...
class MIDTP(Protocol):
    ...
    def __init__(self, segment_size: int = None):
        self.segment_size = segment_size if segment_size is not None else self.MSS
    ...
    def send_file(
        ...
        Args:
            ...
            host: 수신자 호스트 주소
            port: 수신자 포트 번호
            buffer_size: 세그먼트 크기 (헤더 포함) - 하위 호환성 유지
            interval: 세그먼트 간 전송 간격 (테스트용)
            segment_size: 세그먼트 크기 (명시적 설정)
        """
        # 세그먼트 크기 결정
        actual_segment_size = (
            segment_size if segment_size is not None else self.segment_size
        )
        ...
        logger.info(f"세그먼트 크기: {actual_segment_size}")

        chunk_size = actual_segment_size - HEADER_SIZE
        ...
        try:
            ...
            # 파일 정보 전송 (세그먼트 크기 + 총 세그먼트 수 + 파일명)
            file_info = struct.pack(
                "!II256s", actual_segment_size, total_segments, filename.encode()[:256]
            )
            ...
            while not done:
                ...
                logger.info(f"\n{'='*50}")
                logger.info(
                    f"에폭 {epoch_count} 시작 - 전송할 세그먼트: {len(num_seq)}개"
                )
                ...
                try:
                    ...
                    if len(ack_array) == 0:
                        ...
                        logger.info(
                            f"누락 목록: {num_seq[:10]}{'...' if len(num_seq) > 10 else ''}"
                        )
        ...
        try:
            while True:
                try:
                    ...
                    segment_size_recv, total_segments, filename = struct.unpack(
                        "!II256s", data[:264]
                    )
                    ...
                    logger.info(
                        f"파일 {filename} 수신 시작 (총 {total_segments}개 세그먼트, 세그먼트크기: {segment_size_recv})"
                    )
                    ...
                    while True:
                        try:
                            ...
                            data, _ = server_socket.recvfrom(segment_size_recv)

                            # 헤더에서 seq_num과 seq_last 추출 (알고리즘 2, line 7)
                            seq_num, seq_last_recv = struct.unpack(
                                "!II", data[:HEADER_SIZE]
                            )
                            ...
                            if seq_last is None:
                                ...
                                epoch_count += 1
                                logger.info(
                                    f"에폭 {epoch_count} 시작 (seq_last={seq_last})"
                                )
                            ...
                            if seq_num == seq_last:
                                ...
                                if len(missing) == 0:
                                    ...
                                    logger.info(
                                        f"누락 목록: {missing[:10]}{'...' if len(missing) > 10 else ''}"
                                    )

                                    ack_array = array.array("i", missing)
                                    server_socket.sendto(
                                        ack_array.tobytes(), client_address
                                    )
                    ...
                    if len(received) == total_segments:
                        ...
                        logger.error(
                            f"전송 실패: {len(received)}/{total_segments} 세그먼트만 수신"
                        )
