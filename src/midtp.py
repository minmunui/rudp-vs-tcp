...
class MIDTP(Protocol):
    ...
    def send_file(self, filename: str, host: str, port: int = 9999, 
                  buffer_size: int = MTU_DATA_SIZE, interval: float = 0.0005):
        """
        ...
        try:
            ...
            for i in range(total_chunks):
                ...
                print(f"\r전송 진행률: {progress:.1f}% (패킷 {i + 1}/{total_chunks})", end='')
            ...
            while not transfer_complete and retry_count < max_retries:
                try:
                    ...
                    missed_seqs = wait_ack(client_socket, timeout=3.0)
                    ...
                    try:
                        ...
                        try:
                            ...
                            if seq_num == METADATA_SEQ:
                                ...
                                logger.info(f"메타데이터 수신: {metadata['filename']}, "
